...
const OngoingTrips = ({ userTrips, refreshTrips }) => {
  ...
  useEffect(() => {
    const ongoingFiltered = userTrips.filter((trip) => {
      const arrival = new Date(trip.date_of_arrival);
      const departure = new Date(trip.date_of_return);
      arrival.setHours(0, 0, 0, 0);
      departure.setHours(0, 0, 0, 0);
      return arrival <= today && departure >= today;
    });
    setTrips(ongoingFiltered);
  }, [userTrips]);
  ...
  return (
    <div className="p-4 sm:p-6">
      <h2 className="text-2xl font-bold mb-4 text-center sm:text-left">Ongoing Trips</h2>
      {trips.length === 0 ? (
        <p className="text-gray-500 text-center">No ongoing trips right now.</p>
      ) : (
        trips.map((trip) => (
