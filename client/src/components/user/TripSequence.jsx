...
const TripSequence = ({ trip, onClose }) => {
    ...
    useEffect(() => {
        if (trip_sequence.length > 0) {
            ...
            trip_sequence.forEach((item, index) => {
                ...
                initialBudgets[index] = item.budget || "";
                initialExpenses[index] = item.expense || "";
            });
            setSelectedDestinations(initialDestinations);
            setSelectedDates(initialDates);
            setSelectedBudgets(initialBudgets);
    ...
    const handleBudgetChange = (index, event) => {
        setSelectedBudgets(prev => ({ ...prev, [index]: event.target.value }));
    };

    const handleExpenseChange = (index, event) => {
        setSelectedExpenses(prev => ({ ...prev, [index]: event.target.value }));
    };
            ...
            <div className="p-6 bg-white rounded-lg shadow-md max-w-lg w-full mx-auto border border-gray-300 relative">
                ...
                <div className="space-y-4">
                    {destinations.map((_, index) => (
                        <div key={index} className="flex flex-col space-y-2">
                            <div className="flex flex-wrap gap-2 items-center">
                                <select 
                                    className="p-2 border border-gray-300 rounded-lg w-[48%] sm:w-[23%]" 
                                ...
                                <input 
                                    type="date" 
                                    className={`p-2 border rounded-lg w-[48%] sm:w-[23%] ${dateErrors[index] ? 'border-red-500' : 'border-gray-300'}`} 
                    ...
                    <div className="flex justify-between mt-4">
                        <button 
                            className="bg-black text-white px-4 py-2 rounded-lg font-semibold shadow-md"
                            onClick={() => onClose(false)}
                        >
                            Close
                        </button>
                        <button 
                            className="bg-[#f2b50d] text-black px-4 py-2 rounded-lg font-semibold shadow-md hover:bg-yellow-500"
